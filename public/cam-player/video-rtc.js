...
export class VideoRTC extends HTMLElement {
  ...
  set src(value) {
    ...
    if (value.startsWith("http")) {
      value = wsProto + value.substring(value.indexOf("://") + 3);
    } else if (value.startsWith("/")) {
      value = wsProto + location.host + value;
    }

    // 🔥 MUHIM: ichki IP ni proxyga o‘tkazamiz
    if (value.includes("172.16.55.12:1984")) {
      const url = new URL(value);
      value = `${wsProto}${location.host}/camera-ws${url.search}`;
    }
    ...
    this.onmessage["webrtc"] = (msg) => {
      switch (msg.type) {
        case "webrtc/candidate":
          if (this.mode.includes("webrtc/tcp") && msg.value.includes(" udp "))
            return;
    ...
    if (this.pc) {
      ...
      if (stream.getVideoTracks().length > 0) {
        ...
        const isH265Supported =
          this.pc.remoteDescription.sdp.includes("H265/90000");
