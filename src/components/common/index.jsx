    ...
    >
      ...
      >
        <svg viewBox="0 0 80 40" style={{ width: "100%", height: "100%" }}>
          <polyline
            points="0,30 15,22 30,26 45,15 60,18 75,10 80,12"
            fill="none"
            stroke={color}
            strokeWidth="2"
          />
...
export const StatBox = ({ label, value, color = "primary.main" }) => (
  <Box>
    <Typography
      sx={{ fontFamily: "'Arial',san-serif", fontSize: "0.9rem", color }}
    >
      {value}
    </Typography>
    <Typography
    ...
    <Box sx={{ p: 2 }}>
      {Array.from({ length: rows }).map((_, i) => (
        <Skeleton
          ...
          variant="rectangular"
          height={42}
          sx={{ mb: 1, bgcolor: theme.palette.divider, borderRadius: 1 }}
        />
      ))}
    </Box>
  );
