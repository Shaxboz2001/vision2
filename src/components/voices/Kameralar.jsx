...
async function getKameralar() {
  const res = await fetch(`${CAMERA_ADMIN_API}/api/cameras`);
  if (!res.ok) throw new Error("Kameralarni olishda xatolik");
  return res.json();
}
...
async function createKamera(payload) {
  const res = await fetch(`${CAMERA_ADMIN_API}/api/cameras`, {
    method: "POST",
  ...
  if (!res.ok) {
    ...
    throw new Error(text || "Kamera qo‘shishda xatolik");
  }
...
function AddCameraDialog({ open, onClose, onSubmit, loading }) {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ fontWeight: 900 }}>Yangi kamera qo‘shish</DialogTitle>
...
export default function Kameralar() {
  ...
  const handleAddCamera = async (form) => {
    try {
      ...
      console.error(err);
      alert(
        "Kamera qo‘shishda xatolik bo‘ldi. Backend 8011 ishlayotganini tekshir.",
      );
    } finally {
      setAdding(false);
    }
